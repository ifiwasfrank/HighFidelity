...
// Provider e Contract (con logging per debug)
let contract = null;
try {
  console.log("Caricamento provider...");
  const provider = new ethers.JsonRpcProvider(process.env.BASE_RPC);
  console.log("Caricamento wallet...");
  const wallet = new ethers.Wallet(process.env.PRIVATE_KEY, provider);
  console.log("Caricamento contract...");
  const abi = ["function mint(address to, uint256 amount) public"];
  contract = new ethers.Contract(process.env.CONTRACT_ADDRESS, abi, wallet);
  console.log("Contract caricato correttamente");
} catch (err) {
  console.error("ERRORE CRITICO NEL WALLET/CONTRACT:", err.message);
  // Continua per non bloccare, ma mint non funzionerà
}
...
app.get('/frame', (req, res) => {
  console.log('Richiesta /frame ricevuta');
  ...
  `.trim());
  console.log('HTML del Frame inviato con successo');
});
  ...
  if (userData[fid]?.lastShare && now - userData[fid].lastShare < 86400000) {
    ...
    res.send(`
      ...
      <head>
        ...
        <meta property="fc:frame:post_url" content="https://high-fidelity-six.vercel.app/frame" /> <!-- Back -->
